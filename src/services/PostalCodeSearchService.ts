 ...
 */
export class PostalCodeSearchService implements IPostalCodeSearchService {
   ...
   */
  async searchByMunicipality(municipalityName: string): Promise<PostalCodeResult> {
    // 入力検証
    const validation = this.validateMunicipalityName(municipalityName);
    if (!validation.isValid) {
      throw new ValidationError(validation.errorMessage || '無効な入力です');
    }
    ...
    if (postalCodes.length === 0) {
      throw new ValidationError('指定された市区町村が見つかりません');
    }

    // 都道府県名と市区町村名を分離（可能な場合）
    const { prefectureName, municipalityName: extractedMunicipality } =
      this.extractPrefectureAndMunicipality(sanitizedInput);
   ...
   * @returns 都道府県名と市区町村名
   */
  private extractPrefectureAndMunicipality(input: string): {
   ...
   * @returns 存在するかどうか
   */
  async checkMunicipalityExists(municipalityName: string): Promise<boolean> {
    try {
      const dataSource = this.dataSourceManager.getDataSource();
      ...
      return municipalities.some(
        m =>
          m.municipalityName.includes(municipalityName) ||
